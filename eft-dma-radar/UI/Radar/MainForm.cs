...
namespace eft_dma_radar.UI.Radar
{
    public sealed partial class MainForm : Form
    {
                    ...
                    if (exits is not null)
                    {
                        foreach (var exit in exits)
                        {
                            ...
                            }
                            {
                                ...
                                // Apply a rotation transformation to the canvas
                                ApplyRotationForText(canvas, exitPosition.X, exitPosition.Y, _rotationDegrees);
        ...
        {
            UpdateTextBoxColor(textBox_VischeckVisColorPMC);
        ...
        {
            UpdateTextBoxColor(textBox_VischeckInvisColorPMC);
        ...
        {
            UpdateTextBoxColor(textBox_VischeckVisColor);
        ...
        {
            UpdateTextBoxColor(textBox_VischeckInvisColor);
        ...
        private void UpdateTextBoxColor(TextBox textBox)
        {
            try
            {
                ...
                Color bgColor = ColorTranslator.FromHtml(textBox.Text);

                // Set the background color of the text box
                textBox.BackColor = bgColor;

                // Determine the contrasting color for the text
                textBox.ForeColor = (bgColor.GetBrightness() < 0.5) ? Color.White : Color.Black;
            }
            catch
            {
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
        ...
        private void DataGridView_Loot_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            // Ensure we're formatting the color column
            if (dataGridView_Loot.Columns[e.ColumnIndex].Name == "Column_ItemColor")
            {
                string colorCode = dataGridView_Loot.Rows[e.RowIndex].Cells["Column_ItemColor"].Value?.ToString();
                ...
                if (!string.IsNullOrEmpty(colorCode))
                {
                    try
                    {
                        ...
                        Color bgColor = ColorTranslator.FromHtml(colorCode);

                        // Apply background color to the color column
                        e.CellStyle.BackColor = bgColor;

                        // Adjust text color to contrast with the background
                        e.CellStyle.ForeColor = (bgColor.GetBrightness() < 0.5) ? Color.White : Color.Black;
                    }
                    catch
                    {
                        // Handle invalid color codes gracefully
